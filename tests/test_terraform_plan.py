...
class TerraformPlanGuardTests(unittest.TestCase):
    ...
    def test_rejects_api_key_or_request_validation_disabled(self):
        plan = valid_plan()
        plan["resource_changes"][3]["change"]["after"]["api_key_required"] = False
        plan["resource_changes"][4]["change"]["after"]["validate_request_body"] = False
        errors = audit(plan, "staging")
