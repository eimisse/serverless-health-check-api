...
| Incoming event logging | structured event log with credential-bearing headers redacted | unit tests + live CloudWatch evidence |
...
| No wildcard IAM actions | repository policy rejects `Action: "*"` | `scripts/check_iam_wildcards.py` |
...
- AWS-generated identifiers such as REST API IDs, deployment IDs, VPC IDs, subnet IDs and VPC endpoint IDs are not customer-named resources.
...
| Automatic Lambda packaging/versioning | Implemented | deterministic `scripts/package_lambda.py`; `GITHUB_SHA` stored as application version |
...
| Invalid POST cannot reach Lambda | Implemented | strict API Gateway request model/validator including `$default` model association |
